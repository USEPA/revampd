...
func (dbService *DatabaseService) FindUnitsByOperatingYear(year int) ([]Unit, error) {
	...
	rows, err := dbService.database.Queryx("SELECT unitid, op_year, facility_name, oris_code, state, epa_region, unit_type_description, stack_ids, op_status, program_code, primary_fuel_type_desc, primary_fuel_group, an_count_op_time, an_gload, an_sload, an_heat_input, an_co2_mass, an_so2_mass, an_nox_mass FROM UNIT_UNIVERSE where op_year = $1", year)
